...
const char *wifi_ssid = "Tha4-2.4G";
const char *wifi_password = "0819065291";

const char *ducouser = "";
const char *rigname = "";

//String ducouser = "";          // Change this to your Duino-Coin ducouser
//String rigname  = "";          // Change this if you want to display a custom rig name in the Wallet

String AcctName = "Siwadol";            //<-- ใส่ชือ User Name ตรงนี
String BrdNm    = "Nam-999";             //<-- ใส่ชือ RigName ตรงนี
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
String FirmwareVer = {"3.1"};
#define URL_fw_Version "https://raw.githubusercontent.com/Thanormsin/FwESP32/master/bin_version32Nam2.txt"
#define URL_fw_Bin "https://raw.githubusercontent.com/Thanormsin/FwESP32/master/fw_32Nam2.bin"
...
void WiFireconnect(void *pvParameters) {
  ...
  for (;;) {
    ...
    if ((wifi_state == WL_CONNECTED) && (wifi_prev_state != WL_CONNECTED)) {
      ...
      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      //hostnm ="Beyond Pool-1";
      //host = "50.112.145.154";
      //port = 6000;
      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      //hostnm ="Beyond Pool-2";
      //host = "50.112.145.154";
      //port = 6002;
      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      //hostnm ="Beyond Pool-4";
      //host = "35.173.194.122";
      //port = 6001;
      //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
void Task1code(void *pvParameters) {
  ...
  for (;;) {
    ...
    while (client_one.connected()) {
      esp_task_wdt_reset();
//      Serial.println("");
      Serial.println("Asking for a new job for user: " + String(ducouser)+ "/" + String(rigname)+ "/" + String(host)+ "/" + String(hostnm)+ "/" + String(port));
//      Serial.println("CORE1 Asking for a new job for user: " +
//                     String(ducouser));
      client_one.flush();
      // Ask for new job     
      client_one.print("JOB," + String(ducouser) + ",ESP32");
      while (!client_one.available()) {
        if (!client_one.connected()) break;
        delay(10);
      }
      ...
      for (unsigned long duco_res_two = 0; duco_res_two < diff_two;
        ...
        if (memcmp(shaResult, job1, sizeof(shaResult)) == 0) {
          ...
          client.print(String(duco_res_two) + "," + String(hashrate_two) +
                       ",Official ESP32 Miner 2.73," + String(rigname) +
...
void setup() {
  ...
  //Delay and Sampling Connect
  Serial.println("Sampling Delay connect to WiFi");
  int i = random(1,25);
  Serial.println(i);
  while(i< 0,i--)
  {
...
//      Serial.print("O");
      ...
      if (lastWiFi == 1) {
        ...
        //WiFi.begin(wifi_ssid1, password1);
